...
function* generateSequenceExample() {
    yield 1;
    yield 2;
    return 3;
}
...
// done: true, если выполнение функции завершено, иначе false.Например, здесь мы создаём генератор и получаем первое из возвращаемых им значений:

function* generateSequenceWorking() {
    yield 1;
    yield 2;
    return 3;
}

let generatorWorking = generateSequenceWorking();

let one = generatorWorking.next();
...
let two = generatorWorking.next();

console.log(JSON.stringify(two)); // {value: 2, done: false}

// И, наконец, последний вызов завершит выполнение функции и вернёт результат return:

let three = generatorWorking.next();
...
let generatorI = generateIterate();

for (let value of generatorI) {
...
let sequence = [0, ...generateSequenceIter()];
...
function* generateSequenceCompositionExample(start, end) {
...
function* generateSequenceComp(start, end) {
    for (let i = start; i <= end; i++) yield i;
}
...
function* generatePasswordCodes() {
    ...
    // 0..9
    yield* generateSequenceComp(48, 57);

    // A..Z
    yield* generateSequenceComp(65, 90);

    // a..z
    yield* generateSequenceComp(97, 122);
...
function* generateSequenceC(start, end) {
    for (let i = start; i <= end; i++) yield i;
}

function* generateAlphaNum() {

    // yield* generateSequenceC(48, 57);
    for (let i = 48; i <= 57; i++) yield i;

    // yield* generateSequenceC(65, 90);
    for (let i = 65; i <= 90; i++) yield i;

    // yield* generateSequenceC(97, 122);
...
let generator5 = generate();

let question2 = generator5.next().value;
